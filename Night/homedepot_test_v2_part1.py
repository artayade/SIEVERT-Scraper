...
def run():
    ...
    driver.find_element(By.CSS_SELECTOR, '#root > div > div:nth-child(3) > div > div > div.col__12-12.col__5-12--sm > div > div > div:nth-child(10) > div > div > div:nth-child(3) > div > div > div.buybox__actions--atc > div > div > div > button').click()
    sleep(5)
...
def main():
    os.makedirs(f'brickseek_temp_json_night_homedepot_{global_date}_part1', exist_ok=True)
    run()
